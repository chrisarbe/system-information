window.onload = function() {
    ...
    document.getElementById("menu_pais").setAttribute("class", "submenu-item active");
};

function pais_borrar(pk) {
        ...
        success: function (data) {
            if(data.status != "1") {
                ...
                Swal.fire({
                    ...
                    confirmButtonText: '<a href="/pais/">Aceptar</a>'
                });
            }
        }
    });
...
function pais_editar(pk) {
    ...
    $.ajax({
        ...
        $('#editar_pais').modal('show');
       });
}

function pais_editar_guardar() {
    const csrftoken = getCookie('csrftoken');
