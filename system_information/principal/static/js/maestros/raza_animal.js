window.onload = function() {
    ...
    document.getElementById("menu_raza_animal").setAttribute("class", "submenu-item active");
};

function raza_animal_borrar(pk) {
    const csrftoken = getCookie('csrftoken');
    $.ajax({
        url: '/raza_animal/borrar/',
        type: 'POST',
        ...
        success: function (data) {
            if(data.status != "1") {
                ...
                Swal.fire({
                    ...
                    confirmButtonText: '<a href="/raza_animal/">Aceptar</a>'
                });
            }
        }
    });
...
function raza_animal_ver(pk) {
    ...
    $.ajax({
        ...
        $('#ver_raza_animal').modal('show');
       });
}

function raza_animal_editar(pk) {
    const csrftoken = getCookie('csrftoken');
    $.ajax({
        url: '/raza_animal/ver/',
        type: 'POST',
        ...
        $('#editar_raza_animal').modal('show');
       });
}

function raza_animal_editar_guardar() {
    const csrftoken = getCookie('csrftoken');
...
function raza_animal_agregar() {
	...
	if (descripcion == "") {
        ...
        $.ajax({
            ...
            success: function (data) {
                $('#agregar_raza_animal').modal('hide');
                if (data.status == "1"){
                    Swal.fire({
                        icon: "success",
                        title: data.message,
